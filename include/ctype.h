 ...
 * islower returns true only for the lowercase letters (abcdefghijklmnopqrstuvwxyz). If islower returns true, it is guaranteed that
 * \sa iscntrl, \sa isdigit, \sa ispunct, \sa isspace will return false for the same character in the same default locale.
 *
 * The behavior is undefined if the value of ch is not representable as unsigned char and is not equal to EOF.
 *
 * @param ch The character to classify
 ...
 * isupper returns true only for the uppercase letters (ABCDEFGHIJKLMNOPQRSTUVWXYZ). If isupper returns true, it is guaranteed that
 * \sa iscntrl, \sa isdigit, \sa ispunct, \sa isspace will return false for the same character in the same default locale.
 *
 * The behavior is undefined if the value of ch is not representable as unsigned char and is not equal to EOF.
 *
 ...
 * conversion rules defined by the default local, i.e. the following uppercase letters (ABCDEFGHIJKLMNOPQRSTUVWXYZ) are replaced
 * with respective lowercase letters (abcdefghijklmnopqrstuvwxyz).
 *
 * The behavior is undefined if the value of ch is not representable as unsigned char and is not equal to EOF.
 *
 * @param ch character to be converted.
 * @return Lowercase version of ch or unmodified ch if no lowercase version is listed in the default locale.
 * */
int tolower(int ch);
 ...
 *
 * Converts the given character to lowercase according to the character
 * conversion rules defined by the default local, i.e. the following lowercase letters (abcdefghijklmnopqrstuvwxyz) are replaced
 * with respective uppercase letters (ABCDEFGHIJKLMNOPQRSTUVWXYZ).
 *
 * The behavior is undefined if the value of ch is not representable as unsigned char and is not equal to EOF.
 *
 * @param ch character to be converted.
 * @return Uppercase version of ch or unmodified ch if no uppercase version is listed in the default locale.
 * */
int toupper(int ch);
 ...
 *
 * @param ch character to be converted.
 * @return The value returned is that of the converted letter, or ch if the conversion was not possible.
 * */
int toascii(int ch);
